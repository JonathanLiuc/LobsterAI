...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    coworkShowContextWindowBar: '显示上下文窗口使用量',
    coworkShowContextWindowBarHint: '在对话框顶部显示 Token 用量进度条，帮助了解当前对话的上下文消耗情况。',
    coworkContextWindowBarLabel: 'Context',
    coworkContextWindowBarTooltip: '预估已使用的 token 占当前模型上下文窗口的比例（按约 4 字符/token 估算）',
    coworkContextWindowBarTooltipExact: '已使用的 token 数（来自 API 实际返回值）占当前模型上下文窗口的比例',
  ...
  en: {
    ...
    coworkShowContextWindowBar: 'Show Context Window Usage',
    coworkShowContextWindowBarHint: 'Display a token usage progress bar above the conversation to help track context consumption.',
    coworkContextWindowBarLabel: 'Context',
    coworkContextWindowBarTooltip: 'Estimated tokens used as a percentage of the model\'s context window (~4 chars/token)',
    coworkContextWindowBarTooltipExact: 'Actual token count (from API response) as a percentage of the model\'s context window',
